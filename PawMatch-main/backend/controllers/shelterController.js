    ...
    try {
        ...
        const shelterRes = await db.query(`
            SELECT 
                ...
                COALESCE(s.organization_name, u.shelter_name, u.name, 'Shelter') as shelter_name, 
                COALESCE(s.contact_number, u.phone_number, 'Not provided') as phone_number,
                COALESCE(s.shelter_description, u.shelter_description, 'No description provided.') as shelter_description,
                COALESCE(s.shelter_address, u.shelter_address, 'Address not provided') as shelter_address,
                s.shelter_logo_url, 
                s.shelter_banner_url, s.shelter_social_links, s.shelter_website, 
                s.shelter_tagline, 
                s.verification_status,
                COALESCE(s.created_at, u.created_at) as created_at,
                (SELECT COUNT(*) FROM adoptions a JOIN pets p ON a.pet_id = p.id WHERE p.shelter_id = u.id AND a.status IN ('active', 'approved', 'completed')) as adoption_count
            FROM users u
